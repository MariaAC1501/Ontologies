...
CONFIG_FILE="${RUN_DIR_REL}/ontocast_500_config.env"
...
ensure_pi_codex_proxy() {
  ...
  : > "${PI_CODEX_PROXY_LOG}"
  PI_AI_COMPAT_PATH="${PI_AI_COMPAT_PATH}" \
  PI_CODEX_PROXY_HOST="${PI_CODEX_PROXY_HOST}" \
  PI_CODEX_PROXY_PORT="${PI_CODEX_PROXY_PORT}" \
  nohup node "${REPO_ROOT}/tools/pi_codex_openai_proxy.mjs" >> "${PI_CODEX_PROXY_LOG}" 2>&1 &
  local proxy_pid=$!
